...
2. [x] **D-C** — aprovado: `RatingRead.match_id`/`ReportRead.match_id` (string solta) substituídos por `RatingRead.match: MatchRef`/`ReportRead.match: MatchRef | None`.
3. [x] **D-D** — aprovado: `create_match` agora emite automaticamente uma `Message(type=system)` ao criar a partida.

**Decisão tomada (D-B):** o campo interno do model `Rating.rated_user_id` (coluna real, FK) **não mudou** — só o schema de resposta da API (`RatingRead`) passou a expandir o relacionamento. Reforça a separação já estabelecida nas fases anteriores entre modelo de persistência e contrato de API exposto.

**Resultado alcançado (D-B):** `pytest` (105 passed, 99.07% cobertura — sem mudança, `test_ratings.py` já cobria o campo alterado), `ruff check`, `black --check` e `mypy app` (strict) todos verdes. Validado manualmente com `uvicorn` local: `/openapi.json` confirma `RatingRead.properties` com `rated_user` (não mais `rated_user_id`); `GET /users/user-1/ratings` contra o banco populado pelo seed retornou `rated_user` totalmente expandido (`id`, `name`, `average_rating`, `matches_played`, etc.).

**Decisão e implementação (D-C):** novo schema `MatchRef` (`id, title, sport, date`) em `app/schemas/match.py`. `RatingRead.match_id` (obrigatório, `Rating.match_id` é FK não-nula) virou `RatingRead.match: MatchRef`; `ReportRead.match_id` (opcional, `Report.match_id` é FK nula) virou `ReportRead.match: MatchRef | None`. Populado em `rating_service.py::build_rating_read` e `report_service.py::build_report_read` via `MatchRef.model_validate(...)` a partir da relação `.match` já carregada — sem query extra. Nenhuma mudança de model/migration necessária (`alembic check` confirmou "No new upgrade operations detected"). Testes atualizados (`test_ratings.py`, `test_reports.py`) para checar `body["match"]["id"]`/`body["match"] is None` em vez de `match_id`.

**Decisão e implementação (D-D):** aprovado emitir a mensagem de sistema. `app/services/match_service.py::create_match` agora insere uma segunda linha em `messages` (`sender_id=organizer.id`, `type=MessageType.SYSTEM`, texto fixo `"Partida criada. Bem-vindos!"`) logo após criar a partida, na mesma unidade lógica (dois `commit()` sequenciais — a partida precisa existir primeiro para satisfazer a FK `messages.match_id`). Escolhido o organizador como `sender_id` porque `Message.sender_id` é FK obrigatória para `users.id` — não há conceito de mensagem "sem remetente" no model atual. Novo teste `test_create_match_emits_system_message` (`test_matches.py`) cobre o fluxo ponta a ponta via API (`POST /matches` seguido de `GET /matches/{id}/messages`).

**Resultado alcançado (D-C + D-D):** `pytest` (106 passed, 99.08% cobertura), `ruff check`, `black --check`, `mypy app` (strict) e `alembic check` todos verdes. Validado manualmente com `uvicorn` local: `/openapi.json` confirma `MatchRef` referenciado em `RatingRead.match` (obrigatório) e `ReportRead.match` (`anyOf` com `null`); fluxo real via `POST /auth/register` → `POST /matches` → `GET /matches/{id}/messages` confirmou a mensagem de sistema (`type: "system"`, texto padrão, `sender` = organizador) sendo criada automaticamente.

**Pendente para encerrar a Fase 12 (ver `queue.md`):** itens de infraestrutura (CORS/variáveis de produção, decisão de hospedagem, purge de `refresh_tokens`, avaliação de "logout de todos os dispositivos") e regenerar/confirmar `/openapi.json` com o front. D-B/D-C/D-D (as três decisões de contrato) estão 100% concluídas.

**Branch:** `feature/fase-12-contrato`, cortada de `dev`, ainda não mergeada.

### Itens de infraestrutura da Fase 12 (sessão 22, 2026-07-08)

1. [x] **Item 5 — regenerar `/openapi.json` e confirmar com o front:** servidor local (`uvicorn`) subido nesta branch (commit `46ca52c`) e `/openapi.json` regenerado e inspecionado diretamente. Confirmado no schema real: `RatingRead.rated_user: PublicProfileRead` (D-B), `RatingRead.match: MatchRef` e `ReportRead.match: MatchRef | null` (D-C) — tudo batendo com o que este documento já descrevia como implementado. `../squadup-app/.status/backend-contract.md` (repositório do front — na verdade chamado `squadup-app` neste ambiente, não `../front` como o `roadmap.md` cita) recebeu uma nota de topo confirmando D-B/D-C/D-D aplicadas e validadas, sem precisar reescrever o documento inteiro (commit `b149c96` no repositório do front).
2. [x] **Item 1 — CORS/variáveis de ambiente de produção:** `cors_origins` (`app/core/config.py`) passou a ser configurável via `CORS_ORIGINS` no `.env` (lista separada por vírgula). Implementado com `Annotated[list[str], NoDecode]` + `field_validator(mode="before")` — necessário porque `pydantic-settings` tenta decodificar `list[str]` como JSON antes de qualquer validador rodar; `NoDecode` desliga esse parsing automático e deixa o validator fazer o split manual. Sem a env var definida, mantém os defaults de dev (`DEFAULT_CORS_ORIGINS`). `.env.example` e `README.md` (nova seção "CORS") documentados. Novo `app/tests/test_config.py` (2 casos). A URL real do app publicado (Expo/EAS) ainda não existe — quando existir, só definir `CORS_ORIGINS` no ambiente de produção, sem mudança de código.
3. [x] **Item 3 — rotina de purge de `refresh_tokens`:** nova função `app/services/auth_service.py::purge_expired_refresh_tokens` (remove linhas `revoked = True OR expires_at < now`, retorna a contagem removida), chamada no `lifespan` de `app/main.py` a cada inicialização da API — mesmo padrão já usado por `create_db_and_tables()` no mesmo lifespan. Decisão tomada com o usuário: purge no startup, sem infraestrutura de scheduler dedicada (Celery/cron), suficiente para o volume esperado do MVP. Uso de `sqlmodel.col(RefreshToken.revoked).is_(True)` em vez de `RefreshToken.revoked == True` foi necessário para o `mypy` (strict) não reclamar de `"bool" has no attribute "is_"` — `col()` sinaliza para o SQLModel tratar o atributo como coluna SQLAlchemy, não como o tipo Python declarado. Dois testes novos em `test_auth.py` (`test_purge_removes_expired_and_revoked_tokens`, `test_purge_is_noop_when_no_stale_tokens`), usando a fixture `session` direto no service (sem passar pela API). Dívida técnica correspondente removida de `queue.md`.
...
## Fase 13 — tarefas 1–4 concluídas (sessão 2026-07-28)

A pedido do usuário, retomado o trabalho no backend (destravado desde 2026-07-16, sessão 29 do front). Branch `feature/fase-13-geo-push`, cortada de `dev`.

**1. Migration de coordenadas + filtro geográfico em `GET /matches`:** `Match.latitude`/`Match.longitude` (`float | None`, nullable, aditivo — migration `0bb63757da75`); propagados em `MatchCreate`/`MatchRead` (`MatchDetailRead` herda). Novo `app/services/geo.py::haversine_km` (fórmula de Haversine pura, sem dependência externa). `list_matches` (`match_service.py`) ganhou `lat`/`lng`/`radius_km` (default 20km, expostos como query params em `read_matches`): quando `lat`/`lng` vêm juntos, filtra por raio e ordena por distância crescente, calculando tudo em Python sobre o resultado já filtrado pelos outros critérios (sem PostGIS, conforme decisão já registrada) — extraído para `_filter_matches_by_radius` por Cognitive Complexity. Novo campo `MatchRead.distance_km` (`float | None`), preenchido só na busca geográfica, para o front não precisar recalcular no cliente.

**2. Tabela `push_tokens` + registro:** model `PushToken` (`id`, `user_id → users.id`, `token` unique, `created_at`) — migration `4ff7c6bb6aa9`, encadeada após a de coordenadas. `app/services/push_token_service.py`: `register_push_token` (idempotente por `token` único; se o token já pertencer a outro usuário — dispositivo compartilhado, conta trocada — realoca o dono em vez de duplicar ou falhar), `revoke_all_push_tokens`, `get_push_tokens_for_users` (busca em lote por lista de `user_id`). Novo endpoint `POST /users/me/push-token` (`204`, `PushTokenCreate{token}`).

**3. `notification_service.py`:** `send_push(tokens, title, body, data)` chama a Expo Push API (`https://exp.host/--/api/v2/push/send`) via `httpx.post` direto — **desvio do plano original**: `expo-server-sdk` não existe como pacote Python maduro (é um pacote Node/JS); a alternativa já prevista no próprio plano ("chamar a API via `httpx`, já dependência existente") foi a escolhida. `httpx` promovido de dependência de teste para runtime no `requirements.txt`. Nunca propaga exceção (`except httpx.HTTPError: logger.warning(...)`, D-Push-4) — decisão de design: os tokens dos destinatários são resolvidos e a lista já fechada **antes** de agendar o `BackgroundTasks`, dentro da mesma sessão/transação da requisição principal, para não reabrir uma `Session` do SQLModel depois que a dependency já fechou a conexão (`get_session` usa `with Session(engine) as session`, que é encerrada antes do `BackgroundTasks` rodar) — `send_push` recebe só uma `list[str]` de tokens já prontos, sem acesso a banco.

**4. Disparo nos 3 eventos:** `message_service.create_message` (novo parâmetro opcional `background_tasks`) notifica organizador + participantes `confirmed`, exceto quem enviou; `match_service.approve_participant` notifica o usuário aprovado; `match_service.close_match` notifica participantes `confirmed`, exceto o organizador que encerrou. Roteadores (`matches.py`, `messages.py`) passaram a injetar `BackgroundTasks` e repassar ao serviço.

**Revogação de push tokens em logout:** `auth_service.revoke_all_refresh_tokens` (chamada por `POST /auth/logout-all`) agora também chama `revoke_all_push_tokens` — decisão consciente de **não** replicar isso em `revoke_refresh_token`/`POST /auth/logout` (logout de um único dispositivo): o contrato de `POST /users/me/push-token` não associa o token a uma sessão/refresh token específico, então não há como saber com segurança qual push token pertence ao dispositivo que está saindo sem arriscar remover o de outro dispositivo ainda logado do mesmo usuário. Documentado na docstring da função e em `roadmap.md` §19.

**Testes novos (25):** `test_geo.py` (3), `test_notification_service.py` (3, Expo mockado via `monkeypatch.setattr(httpx, "post", ...)`, nunca bate na rede real), `test_push_token_service.py` (6, unit-level via fixture `session`), mais casos em `test_matches.py` (7: coordenadas opcionais no create, filtro por raio, ordenação por distância, exclusão de partida sem coordenadas, `radius_km` sozinho não filtra, push em `close`/`approve`), `test_messages.py` (1, push em nova mensagem) e `test_auth.py` (1, push tokens removidos em `logout-all`, usando a fixture `db_client` para inspecionar a tabela `push_tokens` diretamente).

**Resultado alcançado:** `pytest` (138 passed, era 113; 99.20% cobertura, era 96.26%), `ruff check` limpo, `black --check` limpo, `mypy app` (strict) limpo — precisou de um ajuste de narrowing em `list_matches` (checar `lat is not None and lng is not None` diretamente na chamada, em vez de reusar a variável booleana `geo_search`, mesma limitação de fluxo do mypy já documentada como D9 no front. `bandit -c pyproject.toml -r app --exclude app/tests -ll` (mesma invocação do CI) sem issues. Migrations aplicadas e sem drift (`alembic upgrade head` + `alembic check`). `/openapi.json` conferido via `TestClient` (sem subir `uvicorn` real): `GET /matches` expõe `lat`/`lng`/`radius_km`, `POST /users/me/push-token` presente, `/health` 200. `README.md` atualizado (seções "Partidas", nova "Notificações push", "Modelo de dados", "Roadmap"), `requirements.txt` comentado.
