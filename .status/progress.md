...
**Branch:** `feature/fase-11-hardening`, cortada de `dev`, commit `4160a01` (ainda sem PR aberto). Itens ainda pendentes da fase: revisão de cobertura de testes, documentação OpenAPI, CORS/ambiente de produção, decisão de hospedagem, integração incremental do front via React Query — ver `queue.md` (seção "Checkpointer" tem o estado exato para retomar).
