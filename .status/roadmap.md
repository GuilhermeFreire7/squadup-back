...
| Fase 15 | Dívidas técnicas e evolução de escopo (T2–T6) | 🟢 Concluída — mergeada em `dev` via PR #54 (`ed14b3f`), sincronizada com `origin/dev`; `main` ainda não promovida (9 commits atrás). Ver §20 |
...
>
> **Atualização (2026-07-28): tarefas 1–4 concluídas e mergeadas em `dev` (PR #50)**, na branch
> `feature/fase-13-geo-push`.
> Resumo: migration `latitude`/`longitude` em `Match` (nullable, aditiva); `GET /matches` ganhou
> `lat`/`lng`/`radius_km` (default 20km), com filtro/ordenação por distância via Haversine em
> Python (`app/services/geo.py`) quando `lat`/`lng` vêm juntos — `distance_km` novo no
> `MatchRead`; tabela `push_tokens` + `POST /users/me/push-token` (idempotente, realoca o token
> se já pertencer a outro usuário); `notification_service.py` (Expo Push API via `httpx` direto,
> nunca propaga exceção) disparado via `BackgroundTasks` nos 3 eventos de D-Push-2 (nova
> mensagem, participação aprovada, partida encerrada). `POST /auth/logout-all` agora também
> revoga todos os push tokens do usuário — `POST /auth/logout` (single-device) deliberadamente
> não faz o mesmo, ver nota em `auth_service.revoke_all_refresh_tokens`. 138 testes (25 novos),
> 99.2% cobertura, `ruff`/`black`/`mypy --strict`/`bandit` verdes.
>
> **Atualização (2026-07-28, mesma sessão, continuação):** `dev` promovida para `main` pela
> primeira vez (fast-forward, sem conflitos) e a migration desta fase confirmada rodando em
> produção (Railway, auto-deploy em `dev`) — validado via `GET /health` e
> `GET /matches?lat=...&lng=...&radius_km=...`, ambos `200`. O front concluiu suas etapas 5–7
> (geolocalização e push reais) na mesma data. **Falta só a etapa 8** (hardening ponta a ponta
> em dispositivo físico), a única pendência restante de toda a fase, em ambos os repositórios.
>
> **Atualização (2026-07-30, sessão 31 deste repositório — sincronização, sem código novo):** o
> front rodou mais 4 sessões (32–35) desde a última sincronização aqui. A etapa 8 (hardening real
> em dispositivo físico) começou na sessão 34 do front e achou 7 bugs — **todos do lado do
> front** (D28–D34: telas com dados mockados, dependência nativa faltando, teclado cobrindo
> campo, sem retry de rede, e dois mismatches de validação de cliente vs. contrato real). Os dois
> mismatches (senha 6 vs. `min_length=8`, e-mail permissivo vs. `EmailStr`) **confirmam que os
> schemas Pydantic deste backend (`RegisterRequest`) sempre estiveram corretos** — nenhuma
> mudança de contrato foi necessária aqui; ver `progress.md` §"sincronização com o hardening..."
> para o detalhe completo. Build EAS mais recente do front, `fa25bd21`, **segue sem confirmação
> de teste ponta a ponta pelo usuário** — a etapa 8 continua sendo o único item restante de toda
> a Fase 13/14, em ambos os repositórios, exatamente como no fim da sessão 30. Separadamente, o
> front também rodou uma sessão de limpeza de dívidas técnicas de baixa prioridade (sessão 35,
> branch `chore/tech-debt-cleanup`, ainda não mergeada) — sem qualquer efeito neste repositório.
...
> data prevista/a decidir). Branch `feature/fase-15-dividas-tecnicas`, mergeada em `dev` via
> **PR #54** (`ed14b3f`, confirmado sessão 32) — `main` ainda não promovida. Detalhe
> tarefa-a-tarefa em `progress.md` §"Fase 15".
