...
| Fase 13 | Geolocalização real e notificações push | 🟡 Em andamento — tarefas 1–4 (backend) concluídas e mergeadas em `dev` via PR #50 (2026-07-28); front concluiu geolocalização e push (etapas 5–7, sessões 31–33) — falta só a etapa 8 (hardening conjunto — `../squadup-front/.status/roadmap.md` §20), único item restante |
...
> 99.2% cobertura, `ruff`/`black`/`mypy --strict`/`bandit` verdes. Falta só a etapa 8
> (hardening ponta a ponta em dispositivo físico), que depende do front implementar as etapas
> 5–7 primeiro.
